...
const commands = [
  { name: "HELP", description: "Show command reference", run: () => openPalette() },
  { name: "VERIFY", description: "Mark selected evidence verified", run: () => setSelectedStatus("verified") },
  { name: "PENDING", description: "Return selected evidence to pending", run: () => setSelectedStatus("pending") },
  { name: "FILTER ID", description: "Show identity events", run: () => setTypeFilter("identity") },
  { name: "FILTER NET", description: "Show infrastructure events", run: () => setTypeFilter("infrastructure") },
  { name: "FILTER MEDIA", description: "Show media events", run: () => setTypeFilter("media") },
  { name: "FILTER ALL", description: "Clear event type filter", run: () => setTypeFilter("all") },
  { name: "CLEAR", description: "Clear search and all filters", run: clearFilters },
];
...
async function loadCase() {
  try {
    ...
    elements["api-state"].textContent = "SYNCED";
    enableControls();
    renderCase();
  } catch (error) {
    elements["api-state"].textContent = "OFFLINE";
...
function renderCase() {
  ...
  elements["subject-risk"].textContent = subject.risk.toUpperCase();
  ...
  elements.relations.replaceChildren(...subject.relations.map((relation) => {
    ...
    item.append(node("i"), copy, node("em", { "data-risk": relation.risk }, relation.risk.toUpperCase()));
    return item;
  }));
...
function renderTimeline() {
  ...
  elements["timeline-list"].replaceChildren(...events.map((event) => {
    ...
    score.append(node("b", {}, `${event.confidence}%`), node("span", { class: `status-dot ${event.status}` }, event.status.toUpperCase()));
...
function renderEvidence() {
  ...
  if (!event) {
    elements["event-id"].textContent = "NO EVENT";
    elements["evidence-content"].innerHTML = '<div class="loading-block"><b>SELECT AN EVENT</b></div>';
    return;
  }
  ...
  notes.replaceChildren(...(event.notes.length ? event.notes.map((note) => {
    ...
    return item;
  }) : [node("li", { class: "no-notes" }, "NO ANALYST NOTES") ]));
  ...
  verifyButton.textContent = nextStatus === "verified" ? "MARK VERIFIED" : "RETURN TO PENDING";
...
function renderCommands() {
  ...
  const visible = commands.filter((command) => `${command.name} ${command.description}`.toLowerCase().includes(query));
  elements["command-list"].replaceChildren(...visible.map((command) => {
    ...
    button.append(node("b", {}, command.name), node("span", {}, command.description));
    return button;
  }));
}
